classdef CHilbertMultiL < CHilbertL
    ...
    methods (Access = public)
        function obj = CHilbertMultiL(filename)
            if ~exist('filename','var'), filename = []; end
            obj@CHilbertL(filename); %volani konstruktoru nemuze byt v if bloku - ani ve verzi 2016b                
            obj.RjEpoch = [];
        end
        ...
        function obj = Clear(obj)
            ...
            obj.Hfmean  =[];
            obj.HFreq = [];
            if isprop(obj,'HFreqEpochs'), obj.HFreqEpochs = []; end
            obj.CH = [];
            obj.PsyData = {};
            obj.epochData = {};
            obj.els = [];
