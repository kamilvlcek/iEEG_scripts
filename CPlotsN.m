classdef CPlotsN < handle
    ...
    methods (Access = public)
        ...
        function [ispc_cats, ispc_cats_signif, ispc_cats_clust_corr] = ISPCCalculateCats(obj, separate_kats, kats, channels, chanPair_betweenROI, preselected_chnpairs, exclude_chanPair_lowEpochs, time_window, baseline, normalize)  % Sofia from 6.6.2022 
            ...
            %%% input: separate_kats - whether to compute ispc separately for each category (1) or for all trials together (0)
            % kats - indexes of conditions, e.g. [2,3]
            ...
            % preselected_chnpairs = 0 - for all computed chan pairs, if 1 - only for preselected chan pairs (obtained in obj.ISPCSigPairRatio), if 2 - for all sign chan pairs (stored in obj.plotISPC.chanPair_sign_filtered)
            % exclude_ChanPair_lowEpochs = 1; not to compute ispc for chan pairs with number of epochs < 30; 0 - to compute
            % time_window - if to compute average plv for a time window and not for each sample; default 0 (compute for each sample); for computing plv over time window, it should contain 2 values, e.g. [1 0.2], 0.2 s - time window
            % baseline - baseline period to substract to normalize ispc values
            % normalize - if 1, substract baseline period to normalize ispc values, 0 - not
            ...
            tic
            if ~exist('separate_kats', 'var') || isempty(separate_kats)
                separate_kats = 0; % default - to compute ispc for all trials together
            end
            ...
            if ~exist('preselected_chnpairs', 'var') || isempty(preselected_chnpairs)
                preselected_chnpairs = 0; % default all chan pairs, if 1 computes only for preselected chan pairs (obtained in obj.ISPCSigPairRatio), if 2 - for all sign chan pairs (stored in obj.plotISPC.chanPair_sign_filtered)
            end 
            ...
            if ~exist('time_window', 'var') || isempty(time_window)
                time_window = 0; % default - compute plv for each sample
            end
            ...
            if ~exist('normalize', 'var') || isempty(normalize)
                normalize = 0; % default - not substract baseline period    
            end
            ...
            if separate_kats
                ...
                ispc_all_trials_cats = cell(numel(kats),numel(channels),numel(channels)); %{kats x chn x chn} (time x fq x trials), we need all phases data for permutation stat between conditions
            else
                kats = 1;
                obj.plotISPC.ispc_cats_names{1}{1} = 'all trials together'; % used everywhere in plots
                obj.plotISPC.ispc_cats_names{2} = kats;
            end
            
            iepochtime = round(obj.E.epochtime(1:2).*obj.E.fs); % epochtime in samples
            ibaseline =  round(baseline.*obj.E.fs); % baseline in samples
            if numel(time_window) == 2 % define time samples for which compute plv
                time_bin = time_window(2);
                itime_bin = round(time_bin*obj.E.fs); % size of time bin in samples
                itime = abs(iepochtime(1)-ibaseline(2))+1 : itime_bin : size(obj.E.fphaseEpochs, 1); % indexes of time windows in samples after stimulus
                nTpoints = numel(itime)-1;
                obj.plotISPC.time_window = time_window(2); % store to use in ISPCPlot for x axis
            else % just take after stimulus period for computing plv for each sample
                itime = abs(iepochtime(1)-ibaseline(2))+1 : size(obj.E.fphaseEpochs, 1); 
                nTpoints = numel(itime);
            end            
            ...
            for i = 1:numel(kats) % initialize all matrices inside the cells
                ispc_cats{i} = NaN(numel(channels), numel(channels), nTpoints, size(obj.E.fphaseEpochs, 3)); % NaN will be for non computed chan pairs
                ispc_cats_signif{i} = NaN(numel(channels), numel(channels), nTpoints, size(obj.E.fphaseEpochs, 3));
                ispc_cats_clust_corr{i} = NaN(numel(channels), numel(channels), nTpoints, size(obj.E.fphaseEpochs, 3));                
                    ...
                    if ~isa(obj.E, 'CHilbertMulti') || find(obj.E.els >= channels(ichn1),1) == find(obj.E.els >= channels(ichn2),1) %find expression to get the pacient number 
                        for k = 1:numel(kats) % over all conditions
                            if (chanPair_betweenROI==1 && ~(ismember([channels(ichn1), channels(ichn2)], obj.plotISPC.roi_chanpair,'rows')|| ismember([channels(ichn2), channels(ichn1)], obj.plotISPC.roi_chanpair,'rows')))... % if the current pair of chan is not between ROI pair
                                || (preselected_chnpairs==1 && ~(ismember([channels(ichn1), channels(ichn2)], obj.plotISPC.chanPairs_max_sig,'rows')|| ismember([channels(ichn2), channels(ichn1)], obj.plotISPC.chanPairs_max_sig,'rows')))... % if the current pair of chan is not in preselected ones
                                || (preselected_chnpairs==2 && ~(ismember([channels(ichn1), channels(ichn2)], obj.plotISPC.chanPair_sign_filtered{1},'rows')|| ismember([channels(ichn2), channels(ichn1)], obj.plotISPC.chanPair_sign_filtered{1},'rows'))) % if the current pair of chan was not significant for all trials
                                ispc_sign_chan{k}(ichn1,ichn2) = NaN;
                                ispc_sign_chan{k}(ichn2,ichn1) = NaN; % symmetrical position
                            else
                                if separate_kats  % for separate conditions
                                ...
                                else % for all trials together
                                    iEp = obj.E.GetEpochsExclude();
                                    iEpCorr1 = find(squeeze(iEp(channels(ichn1),:))); % indexes of correct epoches (without errors and training) for chan 1 (without distinguishing into conditions)
                                    iEpCorr2 = find(squeeze(iEp(channels(ichn2),:))); % for chan 2
                                    iEp1 = setdiff(iEpCorr1, find(obj.E.RjEpochCh(channels(ichn1),:))); % also exclude the epi epochs for chan 1
                                    iEp2 = setdiff(iEpCorr2, find(obj.E.RjEpochCh(channels(ichn2),:))); % also exclude the epi epochs for chan 2
                                end
                                % for computing ISPC, the epochs should be the same in both channels
                                ...
                                % if to compute plv over each average time window, then first average phase angle difference for these windows
                                if numel(time_window) == 2
                                    ispc_all_averT = zeros(numel(itime)-1, size(ispc_all,2), size(ispc_all,3)); % new matrix with phase difference averaged across time windows
                                    for it = 1:numel(itime)-1
                                        ispc_all_averT(it, :,:) = abs(mean(exp(1i*ispc_all(itime(it):itime(it+1),:,:)),1)); % mean over each time window, time x freq x epochs (then it's ISPC-time)
                                    end
                                    ispc_all = ispc_all_averT;
                                else % just take after stimulus period for computing plv for each sample
                                    ispc_all = ispc_all(itime, :, :);
                                end
                                
                                ispc_ch = abs(mean(exp(1i*ispc_all),3)); % time x freq, computation of ispc over epochs (ISPC-trials) TODO: in case of computing ISPC over time window, averaging across trials would be different and also permut stat                                                                            
                                ispc_cats_n_epochs{k}(ichn1,ichn2) = numel(iEpBoth); % number of epochs for each pair of chan
                                if separate_kats
                                    ispc_all_trials_cats{k,ichn1,ichn2} = ispc_all; %{kats x chn x chn} (time x fq x trials), all phases data for permutation stat between conditions
                                end
                                % compute statistics
                                [ispc_norm, ispc_signif, ispc_clust_corr] = obj.ISPCPermut(ispc_ch, ispc_all, normalize); % permutation statistics with cluster correction
            ...
            if separate_kats
                obj.plotISPC.ispc_all_trials_cats = ispc_all_trials_cats;
            end
        ...
        function [ispc_norm, ispc_signif, ispc_clust_corr] = ISPCPermut(obj, ispc_computed, ispc_all_trials, normalize, n_permutes, threshold) % Sofia from 18.10.2023
            ...
            if ~exist('n_permutes', 'var') || isempty(n_permutes)
                n_permutes = 500; % default, if more e.g. 500, it is becoming very time-consuming
            end
...
%             
%             % substract baseline period from computed ispc - to normalize ispc values            
...
%             
%             % substract baseline period from phase differences from each trial
%             ispc_all_trials_baseline = mean(ispc_all_trials(ibaseline(1)-iepochtime(1)+1 : ibaseline(2)-iepochtime(1),:,:),1); % mean over time (baseline period), freq x trials
%             ispc_all_trials_norm = bsxfun(@minus,ispc_all_trials,ispc_all_trials_baseline); % time x freq x trials
            
%             % take the data only after stimulus
%             ispc_all_trials_afterStim = ispc_all_trials(time_interval, :, :); 
%             ispc_computed_afterStim = ispc_computed(time_interval, :);
            ...
            for permi = 1:n_permutes
                ...
                permuted_vals(permi,:,:) = squeeze(abs(mean(exp(1i*eegtemp),3))); % mean over epochs, permuted plv
...
%             % plot the distribution of permuted values for one freq and one time point (just for debugging)
%             figure(8), clf
%             histogram(permuted_vals(:,10,7),50)
...
%             ispc_clust_corr = padarray(ispc_clust_corr, [time_interval(1)-1 0], 0, 'pre');
            ...
            if normalize
                ...
                ispc_norm = ispc_computed;
            end
            
        end
        function [realPLV_diff, ispc_diff_signif, ispc_diff_clust_corr] = ISPCPermutCatsDiff(obj, ispc_computed_cats, ispc_all_trials_cats, n_permutes, threshold) % Sofia since 11/2023
            %%% computes permutation-based statistic for PLV difference between conditions for one pair of channels (according to Mike X Coneh book, chapter 34)
            %%% the null hypothesis is generated by shuffling trials in conditions
            % ispc_all_trials_cats - {kats}(time x frequency x trials) phase differences between pair of channels from all trials for two conditions
            % ispc_computed_cats - {kats}(time x frequency) computed plv for each condition
            % output: realPLV_diff - PLV difference between 2 conditions for one pair of chan
            % ispc_diff_signif - significant thresholded PLV diff between 2 conditions for one pair of chan: time x freq with values below the threshold set to zero
            % ispc_diff_clust_corr - time x freq, significant PLV diff after the cluster correction (correction for multiple comparisons) 
                     
            if ~exist('n_permutes', 'var') || isempty(n_permutes)
                n_permutes = 1000; % default
            end
            ...
            permuted_vals_diff = zeros(n_permutes, size(ispc_all_trials_cats{1},1), size(ispc_all_trials_cats{1},2)); % n_permutes x time x freq; PLV differences between 2 conditions
            
            % permute
            for permi=1:n_permutes
                for fi=1:size(ispc_all_trials_cats{1},2) % for each frequency
                    
                    % concatenate trials - phase differences for each condition
                    cat1_phases = squeeze(ispc_all_trials_cats{1}(:,fi,:));
                    cat2_phases = squeeze(ispc_all_trials_cats{2}(:,fi,:));
                    concatCats = [cat1_phases, cat2_phases];
                                        
                    % random permutation
                    fakeconds = randperm(size(concatCats,2));
                    
                    % shuffled condition labels
                    fakeconds(fakeconds<size(cat1_phases,2)+1) = 1;
                    fakeconds(fakeconds>1) = 2;
                    
                    % compute PLV over trials for shuffled conditions
                    PLVshufCat1 = abs(mean(exp(1i*concatCats(:,fakeconds==1)),2));
                    PLVshufCat2 = abs(mean(exp(1i*concatCats(:,fakeconds==2)),2));
                    
                    permuted_vals_diff(permi,:, fi) = PLVshufCat2 - PLVshufCat1; % plv difference
                end
            end
            
%             % plot the distribution of H0 values for one freq and one time point (just for checking purposes)
%             figure(8), clf
%             histogram(permuted_vals_diff(:,20,3),50)
            
            % real PLV difference between 2 conditions
            realPLV_diff = ispc_computed_cats{2}-ispc_computed_cats{1}; % time x frequency 
            
            % transform to z values and set to zero values below thershold
            zmap = (realPLV_diff-squeeze(mean(permuted_vals_diff,1)))./squeeze(std(permuted_vals_diff));
            ispc_diff_signif = realPLV_diff;
            ispc_diff_signif(abs(zmap)<norminv(1-threshold))=0;   
            
%             figure(9)
%             T = linspace(obj.E.epochtime(1), obj.E.epochtime(2), size(obj.E.fphaseEpochs,1)); % time on x-axis
%             F = obj.E.Hfmean;
%             subplot(131)
%             contourf(T,F,zmap',40,'linecolor','none')
%             axis square
...
%             contourf(T,F,zmap',40,'linecolor','none')
%             zmapthresh = zmap;
%             zmapthresh(abs(zmapthresh)<norminv(1-threshold))=false;
%             zmapthresh=logical(zmapthresh);
%             hold on
%             contour(T,F,zmapthresh',1,'linecolor','k')
%             axis square
%             title('Thresholded uncorrected Z map')
%             colorbar
            
            % the cluster correction on the permuted data (multiply comparisons correction)
            for permi = 1:n_permutes
                
                % for cluster correction, apply uncorrected threshold and get maximum cluster sizes
                fakecorrsz = squeeze((permuted_vals_diff(permi,:,:)-mean(permuted_vals_diff,1)) ./ std(permuted_vals_diff,[],1) );
                fakecorrsz(abs(fakecorrsz)<norminv(1-threshold))=0;
            ...
            % apply cluster-level corrected threshold
            ispc_diff_clust_corr = realPLV_diff;
            % uncorrected pixel-level threshold
            ispc_diff_clust_corr(abs(zmap)<norminv(1-threshold))=0;
            % find islands and remove those smaller than cluster size threshold
            clustinfo = bwconncomp(ispc_diff_clust_corr);
            clust_info = cellfun(@numel,clustinfo.PixelIdxList);
            clust_threshold = prctile(max_clust_info,100-threshold*100);
            ...
            % remove clusters
            for i=1:length(whichclusters2remove)
                ispc_diff_clust_corr(clustinfo.PixelIdxList{whichclusters2remove(i)})=0;
            end
            
%             subplot(133)
%             contourf(T,F,realPLV_diff',40,'linecolor','none')
%             hold on
%             contour(T,F,ispc_diff_clust_corr',1,'linecolor','k')
%             axis square
%             title('Cluster-corrected PLV map')
%             xlabel('Time (ms)'), ylabel('Frequency (Hz)')
%             colorbar
        
        end
        
        function [ispc_diff_signif_chpairs, ispc_diff_clust_corr_chpairs] = ISPCStatCatsDiff(obj, freq, time_interval, preselected_chnpairs, permutation, alpha)
            %%% computes permutation-based statistic (permutation=1) for PLV difference between conditions for pairs of channels
            %%% permutation=0: or different type of stat using the equation for evaluation of ISPC diff (p. 488 and 491 in Mike X Cohen book and its implementation in CPlotsN.CalculateITPCdiffs)
            % preselected_chnpairs = 0 - for all computed chan pairs, if 1 - only for preselected chan pairs (obtained in obj.ISPCSigPairRatio), if 2 - for all sign chan pairs (stored in obj.plotISPC.chanPair_sign_filtered)
            % freq - frequency range in Hz for which compute stat, e.g. [4 8]
            % time_interval - time window for which compute stat is sec, e.g. [0 0.5]
            % output:
            % ispc_diff_signif_chpairs - significant thresholded PLV diff between 2 conditions: chn1 x chn2 x time x freq with values below the threshold set to zero
            % ispc_diff_clust_corr_chpairs - chn1 x chn2 x time x freq, significant PLV diff after the cluster correction (correction for multiple comparisons)
            tic
            if ~exist('freq', 'var') || isempty(freq)
                freq = [obj.E.Hf(1) obj.E.Hf(end)]; % default, all freq
            end
            
            if ~exist('time_interval', 'var') || isempty(time_interval)
                time_interval = [obj.E.baseline(2) obj.E.epochtime(2)]; % default, the whole period after stimulus (e.g. [0 2])
                itime_interval = round(time_interval(1:2).*obj.E.fs);
                itime_interval = itime_interval(1)+1:itime_interval(2);
            end
            
            if ~exist('preselected_chnpairs', 'var') || isempty(preselected_chnpairs)
                preselected_chnpairs = 0; % default all computed chan pairs, if 1 computes only for preselected chan pairs (obtained in obj.ISPCSigPairRatio), if 2 computes for all sign chan pairs (stored in obj.plotISPC.chanPair_sign_filtered)
            end
            if ~exist('permutation', 'var') || isempty(permutation)
                permutation = 1; % default permutation statistics
            end
            
            if ~exist('alpha', 'var') || isempty(alpha)
                alpha = 0.05; % threshold for pvalue
            end
            
            [~, ifreq] = ismember(freq, obj.E.Hf); % indexes of desired freq in the data
            %             itime_interval = round(time_interval(1:2).*obj.E.fs); % start and end of time interval in samples
            %             iepochtime = round(obj.E.epochtime(1:2).*obj.E.fs); % epochtime in samples
            %             ibaseline = round(obj.E.baseline(1:2).*obj.E.fs);
            % %             itime_interval = abs(iepochtime(1) - itime_interval(1))+1 : itime_interval(2)-iepochtime(1); % the whole time interval in samples
            %             itime_interval = abs(ibaseline(2) - itime_interval(1))+1 : itime_interval(2)-iepochtime(1); % when data is already after stimulus
            
            channels = obj.plotISPC.channels;
            realPLV_diff_chpairs = NaN(numel(channels), numel(channels), numel(itime_interval), diff(ifreq)+1); % chn x chn x time x fq, NaN will be for non computed chan pairs
            ispc_diff_signif_chpairs = NaN(numel(channels), numel(channels), numel(itime_interval), diff(ifreq)+1); %chn x chn x time x fq
            ispc_diff_clust_corr_chpairs = NaN(numel(channels), numel(channels), numel(itime_interval), diff(ifreq)+1); %chn x chn x time x fq
            ispc_diff_sign_chan = zeros(numel(channels), numel(channels)); %chn1 x chn2 to distinguish non-significant and not computed values
            
            fprintf('ISPCStatCatsDiff:    ');
            for ichn1 = 1:numel(channels)
                fprintf('\b\b\b\b\b%5i',ichn1);
                for ichn2 = ichn1:numel(channels) % iterate only through the upper triangular part of the matrix, not to compute the same value twice for each chan pair
                    if preselected_chnpairs==1 && ~(ismember([channels(ichn1), channels(ichn2)], obj.plotISPC.chanPairs_max_sig,'rows')|| ismember([channels(ichn2), channels(ichn1)], obj.plotISPC.chanPairs_max_sig,'rows')) % if the current pair of chan is not in preselected ones
                        ispc_diff_sign_chan(ichn1,ichn2) = NaN;
                        ispc_diff_sign_chan(ichn2,ichn1) = NaN; % symmetrical position
                    elseif preselected_chnpairs==2 && ~(ismember([channels(ichn1), channels(ichn2)], obj.plotISPC.chanPair_sign_filtered{1},'rows')|| ismember([channels(ichn2), channels(ichn1)], obj.plotISPC.chanPair_sign_filtered{1},'rows')) % if the current pair of chan is not significant
                        ispc_diff_sign_chan(ichn1,ichn2) = NaN;
                        ispc_diff_sign_chan(ichn2,ichn1) = NaN; % symmetrical position
                    else
                        if ~isnan(obj.plotISPC.ispc_sign_chanPair{1}(ichn1, ichn2)) && ~isnan(obj.plotISPC.ispc_sign_chanPair{2}(ichn1, ichn2))
                            % compute stat between 2 conditions for this chan pair
                            ispc_cats = {squeeze(obj.plotISPC.ispc_cats{1}(ichn1,ichn2,itime_interval,ifreq(1):ifreq(2))),squeeze(obj.plotISPC.ispc_cats{2}(ichn1,ichn2,itime_interval,ifreq(1):ifreq(2)))};
                            ispc_all_trials_cats = {obj.plotISPC.ispc_all_trials_cats{1,ichn1,ichn2}(itime_interval,ifreq(1):ifreq(2),:), obj.plotISPC.ispc_all_trials_cats{2,ichn1,ichn2}(itime_interval,ifreq(1):ifreq(2),:)};
                            if permutation % permutation stat
                                [realPLV_diff, ispc_diff_signif, ispc_diff_clust_corr] = obj.ISPCPermutCatsDiff(ispc_cats,ispc_all_trials_cats,[],alpha);
                            else % another type of statistics (also implemented for ITPC diff in CalculateITPCdiffs)
                                realPLV_diff = ispc_cats{2}-ispc_cats{1}; % time x frequency
                                n1 = obj.plotISPC.ispc_cats_n_epochs{1}(ichn1,ichn2); % number of epochs for this chan pair and this condition
                                n2 = obj.plotISPC.ispc_cats_n_epochs{2}(ichn1,ichn2);
                                PLV_diff_pvalue = 2*normpdf(abs(atanh(realPLV_diff)/sqrt((1/(n1-3))+(1/(n2-3)))));
                                ispc_diff_signif = realPLV_diff;
                                ispc_diff_signif(PLV_diff_pvalue > alpha)=0;
                                [~, ~, adj_p]=fdr_bh(PLV_diff_pvalue,0.05,'pdep','no'); % fdr correction
                                ispc_diff_clust_corr = ispc_diff_signif;
                                ispc_diff_clust_corr(adj_p > alpha)=0;
                            end
                            realPLV_diff_chpairs(ichn1,ichn2,:,:) = realPLV_diff;
                            ispc_diff_signif_chpairs(ichn1,ichn2,:,:) = ispc_diff_signif;
                            ispc_diff_clust_corr_chpairs(ichn1,ichn2,:,:) = ispc_diff_clust_corr; % cluster corrected significant ispc values
                            if sum(sum(ispc_diff_clust_corr))~=0
                                ispc_diff_sign_chan(ichn1,ichn2) = 1; % assign 1 to pair of chan if their significant ispc values survived after cluster correction
                            end
                            
                            % assign the computed values to the symmetrical positions
                            realPLV_diff_chpairs(ichn2,ichn1,:,:) = realPLV_diff;
                            ispc_diff_signif_chpairs(ichn2,ichn1,:,:) = ispc_diff_signif;
                            ispc_diff_clust_corr_chpairs(ichn2,ichn1,:,:) = ispc_diff_clust_corr;
                            ispc_diff_sign_chan(ichn2,ichn1) = ispc_diff_sign_chan(ichn1,ichn2);
                        else
                            ispc_diff_sign_chan(ichn1,ichn2) = NaN;
                            ispc_diff_sign_chan(ichn2,ichn1) = NaN; % symmetrical position
                        end
            ...
            % put data to the same cells, to plot them using the same function ISPCPlot
            obj.plotISPC.ispc_cats{numel(obj.plotISPC.ispc_cats)+1} = realPLV_diff_chpairs; % 4D: chn1 x chn2 x time x freq
            obj.plotISPC.ispc_cats_signif{numel(obj.plotISPC.ispc_cats_signif)+1} = ispc_diff_signif_chpairs; % 4D: chn1 x chn2 x time x freq
            obj.plotISPC.ispc_cats_clust_corr{numel(obj.plotISPC.ispc_cats_clust_corr)+1} = ispc_diff_clust_corr_chpairs; % 4D: chn1 x chn2 x time x freq
            obj.plotISPC.ispc_sign_chanPair{numel(obj.plotISPC.ispc_sign_chanPair)+1} = ispc_diff_sign_chan; % 2D: chn1 x chn2
            obj.plotISPC.ispc_cats_names{1}{numel(obj.plotISPC.ispc_cats_names{1})+1} = 'PLV difference: diff-same'; % used everywhere in plots
            obj.plotISPC.ispc_cats_names{1, 2}(end+1) = -1; % used everywhere in plots, will mean diff between conditions
            obj.plotISPC.stat_cats_freq = freq; % also save freq and time interval for which stat was computed
            obj.plotISPC.stat_cats_time = [time_interval numel(itime_interval)]; % and number of time points, needed for ISPCPlot
            toc
        end
...
%                 max_bins{k}.time_sec = transpose(round((itime(itime_max)-abs(iepochtime(1)))/obj.E.fs,1)); % real time value in sec relative to stimulus time
                max_bins{k}.time_sec = obj.plotISPC.PlotISPCbins.time(itime_max)';
        ...
        function ratioMatrix = ISPCSigPairRatio(obj, kat, n_max_bins, onlyPlot) % Sofia since 13.11.2023
            % computes binary maps for each chan pair, then averages them across all pairs 
            % plots resulting time-freq figure showing the ratio of significant pairs / total pairs with computed ispc in each bin
            % also finds indexes of chan pairs in n_max_bins with maximum ratio
            % onlyPlot - if 1, only plots the time-freq figure with the ratio of significant pairs from previously computed data (0 - computes again)
            % kat - category for which to compute binary maps, e.g. 2 (del_same); or if the data is computed for all trials, no condition is given
            % TODO: obtain ratio matrix separately for ROI1-ROI2 connections
            
            if ~exist('kat', 'var') || isempty(kat) % if no condition is given, it is assumed that the data is computed for all trials together
                kat = 1;
                ikat = 1;
            else
                ikat = find(kat==obj.plotISPC.ispc_cats_names{1, 2}); % find index of category in the data   
            end
            
            if ~exist('n_max_bins', 'var') || isempty(n_max_bins) % number of bins with maximum ratio of signin chan pairs, from which to take chan indexes
                n_max_bins = 15;
            end
            
            if ~exist('onlyPlot', 'var') || isempty(onlyPlot)
                onlyPlot = 0; % default - computes the time-freq ratio matrix and plots it
            end
            
            if onlyPlot == 0
                ispc_clust_corr = obj.plotISPC.ispc_cats_clust_corr{ikat};  % {kats}(chn x chn x time x fq),  significant ispc values after cluster correction
                binaryMaps = ispc_clust_corr;
                binaryMaps(ispc_clust_corr > 0 | ispc_clust_corr < 0) = 1; % assign 1 to significant values, leave zeros and NaN unchanged
                
                % average binary maps across computed chan pairs
                channelSum = squeeze(sum(sum(binaryMaps, 1, 'omitnan' ), 2, 'omitnan')); % sum along the channel dimensions (1st and 2nd dimensions)
                nonNaN = ~isnan(squeeze(binaryMaps(:,:,1,1))); % find chan pairs with computed ispc (not NaN)
                ratioMatrix = channelSum / (sum(nonNaN(:))/2); % calculate the ratio of significant pairs for each time-frequency bin (divided by 2 as the matrix is symmetrical)
                
                % save data to the object
                obj.plotISPC.ratioMatrix = ratioMatrix;
                obj.plotISPC.binaryMaps = binaryMaps;
                
                % find the maximum ratio and its indices
                [~, maxIndices] = maxk(ratioMatrix(:), n_max_bins);
                
                % convert linear index to subscripts for time and frequency
                [maxTimeIndex, maxFreqIndex] = ind2sub(size(ratioMatrix), maxIndices);
                chanPairs_all_max_bins = [];
                for ib = 1:numel(maxTimeIndex) % for each bin with maximum ratio of signif pairs
                    % find indexes of chan pairs that have significant ispc in this time-freq bin
                    binaryMapMax = binaryMaps(:,:, maxTimeIndex(ib), maxFreqIndex(ib));
                    % find the indices where the value is 1
                    [ch1, ch2] = find(binaryMapMax == 1);
                    chanPairs = [ch1, ch2];
                    chanPairs_all_max_bins = [chanPairs_all_max_bins ; chanPairs];
                end
                chanPairs_max_sig = unique(chanPairs_all_max_bins, 'rows'); % find the repeated rows
                obj.plotISPC.chanPairs_max_sig = chanPairs_max_sig; % save chan pairs to the object
            else
                ratioMatrix = obj.plotISPC.ratioMatrix;
                nonNaN = ~isnan(squeeze(obj.plotISPC.binaryMaps(:,:,1,1))); % find chan pairs with computed ispc (not NaN)
            end
            
            % visualize the matrix
            figure('Name','ISPCSigPairRatio');
            colormap jet;
%             T = linspace(obj.E.epochtime(1), obj.E.epochtime(2), size(obj.E.fphaseEpochs,1));
            T = linspace(obj.E.baseline(2), obj.E.epochtime(2), size(obj.plotISPC.ispc_cats{ikat},3));
            F = obj.E.Hfmean;
            imagesc(T, F, ratioMatrix');
            axis xy;
            xlabel('Time (s)'), ylabel('Frequency (Hz)')
            colorbar;
            title(['significant pairs / total ' num2str((sum(nonNaN(:))/2)) ' pairs of channels for ' obj.plotISPC.ispc_cats_names{1, 1}{ikat}], 'Interpreter', 'none')
        end
                
        function [ispc_cats_intime] = ISPCAverageFreq(obj, freq, signChanPair,preselected_chnpairs, only_significant_values)  % Sofia from 13.6.2022 
            ...
            if ~exist('preselected_chnpairs', 'var') || isempty(preselected_chnpairs)
                preselected_chnpairs = 0;
            end
            
            if ~exist('only_significant_values', 'var') || isempty(only_significant_values)
                only_significant_values = 0; % default - all ispc values, if 1 averages only significant ispc values                
            end
                ...
                for ichn2 = 1:nchannels
                    if ismember([ichn1, ichn2], obj.plotISPC.roi_chanpair,'rows') % if chan pair is between ROI
                        for k = 1:nkats
                            ...
                            if preselected_chnpairs==1 && ~(ismember([ichn1, ichn2], obj.plotISPC.chanPairs_max_sig,'rows')|| ismember([ichn2, ichn1], obj.plotISPC.chanPairs_max_sig,'rows')) % % if the current pair of chan is not in preselected ones
                                continue
                            end
...
%                 % first find out which condition has a larger number of significant channel pairs to use them in the plot
%                 array_sizes = cellfun(@size, obj.plotISPC.PlotChPair.ispc_cats_intime, 'UniformOutput', false);               
%                 numRows = cellfun(@(x) x(1), array_sizes); % extract the number of rows for each matrix
%                 [~, icond] = max(numRows); % index of the matrix (condition) with the maximum number of rows                
%                 chpairs = obj.plotISPC.PlotChPair.ispc_cats_intime{icond, 1}(:,[1 2]); % all channel pairs with ISPC averaged of this condition
%                 obj.plotISPC.PlotChPair.chpairs = chpairs;
                 % find common chpairs for all conditions
                 allchpairs = [];
                 for k=1:numel(obj.plotISPC.PlotChPair.ispc_cats_intime)
                     chpairs = obj.plotISPC.PlotChPair.ispc_cats_intime{k, 1}(:,[1 2]); % all channel pairs with ISPC averaged of this condition
                     allchpairs = [allchpairs; chpairs]; 
                 end                 
                 sorted_chpairs = sortrows(allchpairs); % sort the rows of the matrix                 
                 [~, idx, ~] = unique(sorted_chpairs, 'rows', 'stable'); % find the repeated rows
                 common_chpairs = sorted_chpairs(setdiff(1:size(sorted_chpairs, 1), idx), :);
                 obj.plotISPC.PlotChPair.chpairs = common_chpairs;
            end
            ...
            for k = 1 : numel(kats)
                ...
                if chpair > size(obj.plotISPC.PlotChPair.ispc_cats_intime{k}(:,3:end),1) % if for this condition, chanpair is not significant
                    ISPCchpair_k = zeros(1,size(obj.plotISPC.PlotChPair.ispc_cats_intime{1}(:,3:end),2)); % just curve of zeros
                else
        ...
        function ISPCPlotCatROIMean(obj, ROI1, ROI2, timeFreqPlot, chanpairs_sign) % Sofiia since 13.6.2022
            ...
            if ~exist('timeFreqPlot','var') || isempty(timeFreqPlot)
                timeFreqPlot = 0; % by default, not plot average time-freq plot of raw ispc for all chan pairs in ROI1-ROI2
            end
            
            if ~exist('chanpairs_sign','var') || isempty(chanpairs_sign)
                chanpairs_sign = 0; % by default, plots average time-freq plot of raw ispc for all chan pairs in ROI1-ROI2, othewise - onlu for significant
            end
            ...
            for k = 1:nkats
                ...
                ispc_cats_roi_mean(k,:) = mean(isps_time_k, 1, 'omitnan'); % average across selected pairs of chan, kats x time points
                ispc_cats_roiE(k,:) = std(isps_time_k,[],1,'omitnan')/sqrt(nChanPairs); % std err of mean
            end
            ...
            if timeFreqPlot
                % plotting average time-freq plot of raw ispc for all chan pairs in ROI1-ROI2 for each condition
                figure('Name','ISPCMeanTime-Freq_overROI','Position', [20, 500, 1200, 300]);
                colormap jet;
                F = obj.E.Hfmean;
                if chanpairs_sign
                    chpairs2avg = chpairs; % chan pairs with significnat ispc
                else
                    chpairs2avg = ROI_pairs; % all chan pairs between two ROIs
                end
                for k=1:nkats
                    ispc_roi_data = zeros(size(chpairs2avg,1),size(obj.plotISPC.ispc_cats{k},3), size(obj.plotISPC.ispc_cats{2},4));
                    for ipair = 1:size(ispc_roi_data,1)
                        ispc_roi_data(ipair,:,:) = squeeze(obj.plotISPC.ispc_cats{k}(chpairs2avg(ipair,1), chpairs2avg(ipair,2), :, :)); % one chan pair - time x freq
                    end
                    ispc_roi_mean = squeeze(mean(ispc_roi_data, 1,'omitnan')); % average across chan pairs
                    nchpairs_computed = sum(~isnan(ispc_roi_data(:,1,1)));
                    subplot(1,nkats,k)
                    imagesc(T, F, ispc_roi_mean');
                    axis xy;
                    xlabel('Time (s)'), ylabel('Frequency (Hz)')
                    colorbar;
                    title(['ISPC mean over ' num2str(nchpairs_computed) ' pairs of channels in ' upper(ROI1) ' - ' upper(ROI2) ' for condition: ' obj.plotISPC.PlotChPair.kats_legend{k}], 'Interpreter', 'none')
                end
            end
        end
...
%                 obj.plotISPC.aplha = 0.4; %transparency for non signif IPSC values 1=non transparent, 0= completely transparent
            ...
            if obj.plotISPC.current_cat <= -1 % if we want to plot the PLV diff between conditions, then time and freq range can be different
                T = linspace(obj.plotISPC.stat_cats_time(1),obj.plotISPC.stat_cats_time(2), obj.plotISPC.stat_cats_time(3));
                F = obj.plotISPC.stat_cats_freq(1) : obj.plotISPC.stat_cats_freq(2);
            elseif isfield(obj.plotISPC,'time_window') && ~isempty(obj.plotISPC.time_window) 
                T = obj.E.baseline(2)+obj.plotISPC.time_window : obj.plotISPC.time_window : obj.E.epochtime(2);
                F =  obj.E.Hfmean;
            else
                T = linspace(obj.E.baseline(2), obj.E.epochtime(2), size(obj.plotISPC.ispc_cats{ik},3));
                F =  obj.E.Hfmean;
            end
            ...
            if updateS(1)
                ...
                im1=imagesc(T,F,ispc); % all ispc values               
                sign_mask = ispc_signif~=0;  % apply transparency to the non-signif values
                alphaMask = double(sign_mask); 
                alphaMask(alphaMask==0) = 0.5; % 0.5 of transparency for non-significant values, 1 - for significant
...
%                 im1.AlphaData = imAplhaISPC;
                imagesc(T,F,ispc_clust_corr', [-0.5 0.5])
