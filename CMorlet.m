classdef  CMorlet < CHilbert
    ...
    methods (Access = public)
        ...
        function obj = PasmoFrekvence(obj,freq,channels)
            ...
            if freq(1) < 10 
                time = -2:1/obj.fs:2; %pro nizke frekvence pouziju delsi wavelet
            else
                time = -1:1/obj.fs:1; %pro vyssi frekvence staci wv +- 1s
            end
            s    =  5./(2*pi*freq);   %pet cyklu waveletu
            n_wavelet            = length(time); %513
            n_data               = obj.samples; %data nejsou epochovana
            n_convolution        = n_wavelet+n_data-1; %vic bodu nez maji data, takze budu pridavat 0 na konce
            n_conv_pow2          = pow2(nextpow2(n_convolution)); %nejblizsi vyssi mocnina 2
