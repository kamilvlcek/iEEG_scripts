...
if(~exist('PLV_data','var')) || isempty(PLV_data), PLV_data = 'PLV_VTC-IPL_middle 1.5s delay_vs_bs_all_trials_2024-02.mat'; end
...
for p = 1:numel(pacienti)
    if pacienti(p).todo
        if isfile([basedir pacienti(p).folder '\' subfolder '\' PLV_folder '\' PLV_data])
            ...
            maxplv = max(max(max(squeeze(PLVDelay.plvspctrm(ROI_chanpairs(significant_chanPairs,1),ROI_chanpairs(significant_chanPairs,2), :)))));
            ...
            for i = 1:numel(significant_chanPairs)
                ...
                figure(i); plot(PLVDelay.freq, squeeze(PLVDelay.plvspctrm(ipair(1),ipair(2),:)),'b', 'LineWidth', 1)
                hold on
                plot(PLVBaseline.freq, squeeze(PLVBaseline.plvspctrm(ipair(1),ipair(2),:)),'r', 'LineWidth', 1)
                hold on
                ylim([0 maxplv*1.05])
                yLimits = ylim;
                y = yLimits(2) * 0.1;
                ...
                % plot significance
                ySignifPosValues = ones(1, numel(PLVBaseline.freq)) * NaN; % Initialize with NaN
                ySignifPosValues(significancePos) = y; % Assign y values only at significant indices
                plot(PLVBaseline.freq, ySignifPosValues, '-b', 'LineWidth', 3);
                hold on

                ySignifNegValues = ones(1, numel(PLVBaseline.freq)) * NaN; % Initialize with NaN
                ySignifNegValues(significanceNeg) = y; % Assign y values only at significant indices
                plot(PLVBaseline.freq, ySignifNegValues, '-r', 'LineWidth', 3);

                xlim([PLVBaseline.freq(1) PLVBaseline.freq(end)])
                xlabel('Frequency, Hz')
                ylabel('PLV')
                legend('delay last 1.5 sec', 'baseline 1.5 sec');
                title(['PLV between ' PLVDelay.label{ipair(1), 1} ' ' dataDelay.channelInfo(ipair(1)).ROI ' and ' PLVDelay.label{ipair(2), 1} ' ' dataDelay.channelInfo(ipair(2)).ROI]);

                figi_filename = [fig_filename '_chnpair_' PLVDelay.label{ipair(1), 1} ' ' PLVDelay.label{ipair(2), 1} '_' num2str(i) '.fig'];
