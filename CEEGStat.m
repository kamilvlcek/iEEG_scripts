classdef CEEGStat
    ...
    properties (Access = private)
        ...
        d; %matice EEG dat - TODO - add dimensions
    ...
    methods (Access = public)
        ...
        function [P, ibaseline, iepochtime, itimewindow] = WilcoxBaseline(obj,epochtime,baseline,timewindow,iEp,RjEpCh)
            ...
            WpA = CStat.Wilcox2D(response, baselineA, 1, [], 'mean vs baseline A', ...
                RjEpCh(:, iEp),RjEpCh(:, iEp)); % 1=mene striktni pdep, 2=striktnejsi dep;
            WpB = CStat.Wilcox2D(response, baselineB, a1, [], 'mean vs baseline B', ...
                RjEpCh(:, iEp), RjEpCh(:, iEp));  % 1=mene striktni pdep, 2=striktnejsi dep;
        ...
        function [WpKat, WpKatBaseline] = WilcoxCat(kats, responsekat, baselinekat, rjepchkat, itimewindow, method, Pbaseline)
            ...
            for k = 1: numel(kats)
                 ...
                 if exist('Pbaseline', 'var') && ~isempty(Pbaseline)
                     ...
                     for ch = 1:size(responsekat{k}, 2)
                        % musim baseline signif taky pocitat pro kazdy kanal zvast protoze ji pak zohlednuju mezi kategoriemi
                        WpBA = CStat.Wilcox2D(responsekat{k}(:,ch,:), baselineA(:,ch,:), 0, [], ...
                            ['kat ' num2str(k) ' vs baseline A'], rjepchkat{k}(ch,:), rjepchkat{k}(ch,:));
                        WpBB = CStat.Wilcox2D(responsekat{k}(:,ch,:),baselineB(:,ch,:),0,[], ...
                            ['kat ' num2str(k) ' vs baseline B'], rjepchkat{k}(ch,:), rjepchkat{k}(ch,:));
                        WpKatBaseline{k,1}(:,ch) = max (WpBA, WpBB);
                     end
                     fprintf('... %i \n',ch);
                 else % statistika pro vsechny kanaly najednou  
                     WpBA = CStat.Wilcox2D(responsekat{k}, baselineA, 1, [], ...
                         ['kat ' num2str(k) ' vs baseline A'], rjepchkat{k}, rjepchkat{k});
                     WpBB = CStat.Wilcox2D(responsekat{k}, baselineB, 1, [], ...
                         ['kat ' num2str(k) ' vs baseline B'], rjepchkat{k}, rjepchkat{k});
