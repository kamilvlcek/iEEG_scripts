...
for p = 1:numel(pacienti)
    if pacienti(p).todo 
        if(exist([basedir pacienti(p).folder '\' pacienti(p).data],'file')~=2),   disp(['Data neexistuji: ' pacienti(p).folder '\' pacienti(p).data]); chybasoubor = true; end;
        if(exist([basedir pacienti(p).folder '\' pacienti(p).header],'file')~=2), disp(['Header neexistuje: ' pacienti(p).folder '\' pacienti(p).header]); chybasoubor = true; end;
        if(exist([basedir pacienti(p).folder '\' subfolder '\' pacienti(p).psychopy],'file')~=2),  disp(['Psychopy soubor neexistuje: ' pacienti(p).folder '\' subfolder '\' pacienti(p).psychopy]); chybasoubor = true; end;
        if(exist([basedir pacienti(p).folder '\' subfolder '\' pacienti(p).rjepoch],'file')~=2),   disp(['rjepoch neexistuje: ' pacienti(p).folder '\' subfolder '\' pacienti(p).rjepoch]); chybasoubor = true; end;
    end
end
if chybasoubor 
   error('nenalezeny nektere soubory');
    ...
    if pouzetest, disp('pouze test existence souboru'); return; end; %#ok<UNRCH>
end
