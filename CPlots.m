...
%             if numel(channels) > 20, chnshow = [chnshow ' ...']; end
        ...
        function TimeIntervals(obj,vch,intervals,ylimits, nofile,store) % Sofiia 2020
            ...
            for k=1:numel(kats)
                hold on
                errorbar(intervals(:, 2),M(:,k),E(:,k),'.','LineWidth',2,'Color', barvy(k,:)); % plot errors
                hold on;
                h_mean = plot(intervals(:, 2), M(:,k),style{k},'LineWidth',2,'Color',barvy(k,:),'MarkerSize',10,'MarkerFaceColor', barvy(k,:)); % plot means %'none'
